...
const addAgenda = async (req, res) => {
    ...
    try {
        ...
        // Crear la agenda
        const sqlAgenda = `
            INSERT INTO agenda (ID_Profesional, ID_Especialidad, Fecha_Inicio, Fecha_Fin, Hora_Inicio, Hora_Fin, Duracion_Cita, Estado, Activo)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'Disponible', 1)
        `;
        const [result] = await db.query(sqlAgenda, [profesionalId, especialidadId, fechaInicio, fechaFin, horaInicio, horaFin, duracionCita]);
        ...
        for (const dia of diasTrabajo) {
            await db.query(sqlDiasTrabajo, [agendaId, dia]);
        }
